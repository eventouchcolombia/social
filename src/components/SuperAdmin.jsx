...
const SuperAdmin = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      console.log(
        `✅ ${finalData?.length || 0} eventos encontrados:`,
        finalData
      );
      ...
      const sortedData = finalData
        ? [...finalData].sort((a, b) =>
            a.event_slug.localeCompare(b.event_slug)
          )
        : [];
      ...
      if (finalData.length === 0) {
        showMessage(
          "⚠️ No se encontraron eventos. Verifica las políticas RLS en Supabase.",
          "error"
        );
      }
    } catch (error) {
  ...
  const createEvent = async (e) => {
    ...
    try {
      ...
      if (existing) {
        showMessage(
          "Ya existe un admin con ese email para este evento",
          "error"
        );
        setIsCreating(false);
        return;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 sm:p-6 mb-8">
          ...
          >
            <input
              ...
              onChange={(e) =>
                setNewEvent((prev) => ({ ...prev, email: e.target.value }))
              }
          ...
          <p className="text-gray-400 text-xs sm:text-sm mt-2 break-all">
            El slug del evento será la URL: /
            {newEvent.eventSlug || "evento-slug"}
          </p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4">
            <div className="flex-1">
              ...
              <p className="text-gray-400 text-xs sm:text-sm break-all">
                Debug: isSuperAdmin={isSuperAdmin.toString()}, events.length=
                {events.length}
              </p>
            </div>
              ...
              <button
                onClick={async () => {
                  ...
                  alert(
                    `Debug: ${
                      allData?.length || 0
                    } registros encontrados. Ver consola para detalles.`
                  );
                }}
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <div className="flex gap-2 flex-wrap">
                            ...
                            <button
                              onClick={() =>
                                deleteEvent(event.id, event.event_slug)
                              }
        ...
        <div className="mt-6 sm:mt-8 bg-yellow-500/10 border border-yellow-500/50 rounded-lg p-3 sm:p-4">
          <h3 className="text-yellow-200 font-semibold mb-2 text-sm sm:text-base">
            📋 Proceso para crear eventos:
          </h3>
          <ul className="text-yellow-100 text-xs sm:text-sm space-y-1 sm:space-y-2">
            <li className="break-words">
              • <strong>Paso 1:</strong> Crea un evento ingresando el email del
              admin y un slug único
            </li>
            <li className="break-words">
              • <strong>Paso 2:</strong> El admin debe ir a /{`{slug}`}/admin e
              iniciar sesión
            </li>
            <li className="break-words">
              • <strong>Paso 3:</strong> Usar "Configurar Assets" para subir
              imágenes al AssetWizard
            </li>
            <li className="break-words">
              • <strong>Assets requeridos:</strong> background.png y marco.png
            </li>
            <li className="break-words">
              • <strong>Assets opcionales:</strong> bgchosee.png, bggallery.png,
              adminbg.png
            </li>
            <li className="break-words">
              • <strong>Resultado:</strong> El evento estará disponible en /
              {`{slug}`}
            </li>
          </ul>
        </div>
